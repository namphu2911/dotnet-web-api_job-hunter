...
| Domain Entities              | 8/8           | –                                                                             |
| Domain Enums                 | 3/3           | –                                                                             |
| Domain Repositories          | 1/8           | Company, Job, Skill, Permission, Role, Resume, Subscriber                    |
| Infra Repositories           | 1/8           | Company, Job, Skill, Permission, Role, Resume, Subscriber                    |
| Application Services         | 2/11          | Company, Job, Skill, Permission, Role, Resume, Subscriber, File, Email       |
| API Controllers              | 2/11          | Company, Job, Skill, Permission, Role, Resume, Subscriber, File, Mail, Home  |
| DTO Contracts                | Partial       | Company, Job, Skill, Permission, Role, Resume, Subscriber, File, Email       |
| Response Envelope            | ❌ Chưa có    | `RestResponse<T>` + `FormatRestResponse` + `@ApiMessage` (Java pattern)      |
| Exception Mapping            | Partial       | Thiếu: PermissionException→403, validation field list, StorageException→400  |
| Filter/Query DSL             | ❌ Chưa có    | Tất cả list endpoints (Java dùng SpringFilter DSL)                           |
| Static Resource Serving      | ❌ Chưa có    | `/storage/**` mapping cho uploaded files                                     |
| Config Parity                | Partial       | Upload base-uri, file size limit, mail SMTP config trong appsettings.json    |
| Database Seeding             | ❌ Chưa có    | 34 permissions, SUPER_ADMIN role, admin user                                 |
| File Upload/Download         | ❌ Chưa có    | `FilesController` + `FileService`                                            |
| Email Service                | ❌ Chưa có    | `EmailService` + HTML template + `MailController`                            |
| Scheduled Jobs               | ❌ Chưa có    | Background job gửi email định kỳ cho subscribers                             |
| Permission Enforcement       | Partial       | Khung có, chưa áp `[HasPermission]` vào endpoint nào                        |
...
- `UserRepository.cs` – repository duy nhất đã implement
...
- `Authorization/` – `HasPermissionAttribute`, `PermissionAuthorizationHandler`, `PermissionPolicyProvider`, `PermissionRequirement` *(khung có, chưa áp dụng)*
- `Middleware/GlobalExceptionMiddleware.cs` – xử lý lỗi toàn cục *(còn thiếu một số loại lỗi)*
...
| `_` (generic)                 | 500         | –                           |
| *(thiếu)* StorageException    | –           | ❌ chưa map                 |
| *(thiếu)* PermissionException | –           | ❌ chưa map → cần 403       |
| *(thiếu)* Validation errors   | –           | ❌ chưa trả field error list |
...
**Cần làm trong .NET:**
- Tạo `DatabaseSeeder` class (implements `IHostedService` hoặc gọi từ `Program.cs` sau `app.Build()`)
- Chỉ seed khi bảng trống (kiểm tra count trước)
...
**Cần làm trong .NET:**
- Tạo `IFileService` + `FileService` sử dụng `System.IO`
- Validate extension: pdf, jpg, jpeg, png, doc, docx
- Tên file có timestamp prefix để tránh trùng
- `FilesController`: `POST /files?folder=...` (upload), `GET /files?fileName=...&folder=...` (download)
...
**Cần làm trong .NET:**
- Cài `MailKit` hoặc `System.Net.Mail`
- Tạo HTML email template (Razor template hoặc file HTML đơn giản)
- `IEmailService` + `EmailService`
- `MailController`: `GET /email` trigger gửi email
...
**Cần làm trong .NET:**
- `IHostedService` + `PeriodicTimer` hoặc `Quartz.NET` / `Hangfire`
- Background job định kỳ gọi `ISubscriberService.SendSubscribersEmailJobsAsync()`
...
| `spring.mail.username`                   | *(env var)*           | `Email:Username`                         |
| `spring.mail.password`                   | *(env var)*           | `Email:Password`                         |
| `springdoc.swagger-ui.persist-authorization` | `true`           | Swagger `PersistAuthorization = true`    |

**Trạng thái .NET:** Chưa có các section `FileStorage` và `Email` trong `appsettings.json`.
...
**Trạng thái .NET:** Có khung (`HasPermissionAttribute`, `PermissionAuthorizationHandler`) nhưng **chưa có `[HasPermission]` được gắn vào bất kỳ endpoint nào**.
...
**Trạng thái .NET:** `GET /users` chỉ có `page` và `pageSize` – không có khả năng filter theo field.
...
|-------------------------------------|---------------------------------------------------|-----------------------------------------------------|
| `UsersController` – list endpoint   | `GET /users?page=1&pageSize=20`                   | Thiếu filter DSL (Java có filter=email~'gmail')     |
...
| Permission/AuthZ framework          | `HasPermissionAttribute`, `PermissionAuthorizationHandler` | Chưa áp `[HasPermission]` vào bất kỳ endpoint nào |
